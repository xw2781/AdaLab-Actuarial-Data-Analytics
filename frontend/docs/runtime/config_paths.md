...
- ArcRhoTri generated dataset CSV/JSON sidecar pairs use `projects/<project>/data/generated/<ReservingClassFolder>/<DatasetName>@<OriginLength>@<DevelopmentLength>.csv` with a plain `<DatasetName>.json` sidecar, so shape changes rebuild the length-specific CSV cache while preserving one metadata sidecar per dataset. Dataset sidecars record the current Windows login user in `user` and `modified_by` for Project Instance's `User` column.
