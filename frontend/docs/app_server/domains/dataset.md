...
- Exposes a project/path cached dataset lookup for Project Instance; the server resolves `project_name` plus selected reserving-class path to both `projects/<project>/data/generated/<ReservingClassFolder>` and `projects/<project>/data/manual/<ReservingClassFolder>`, then returns dataset names inferred from `.csv` files and metadata `.json` sidecars plus a folder signature derived from cache-file names, sizes, modification times, and folder existence. Each returned file entry also includes disk last-modified and created timestamps plus any recognized user/created/updated text from a related metadata JSON file, including the `user`/`modified_by` login fields written by dataset sidecar writers. Length-scoped cache filenames such as `<DatasetName>@12@12.csv` or older `<DatasetName>@12@12.json` sidecars are normalized to the base dataset instance name.
<!-- MANUAL:END -->

## Data/State/Caches
<!-- MANUAL:BEGIN -->
- Uses in-memory dataset map and patch payloads.
- ArcRhoTri CSV request targets are `projects/<project>/data/generated/<ReservingClassFolder>/<DatasetName>@<OriginLength>@<DevelopmentLength>.csv`; the matching metadata sidecar is written as plain `<DatasetName>.json` in the same folder and records dataset type/instance labels, `data_format`, `origin_length`, `development_length`, `created`, `updated_at`, and user fields. The reserving-class path is a single filename-escaped folder name using the reversible `_%XX_` rule and is not repeated in the CSV filename.
...
- Cached dataset lookup is read-only and scans both generated and manual folders for the selected reserving-class path, matching `.csv` and `.json` filenames/sidecars back to dataset-name candidates after applying the same server-side folder/file sanitizers used by runtime cache writes. The response includes per-file `size`, `mtime`, and `mtime_ns` fields plus a `folder_signature` for frontend stale-content detection.
<!-- MANUAL:END -->
