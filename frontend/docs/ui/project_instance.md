...
- Cached dataset view is page-local state; changing the selected reserving-class path refreshes the server-side cache-file lookup and cached-file metadata for the new generated/manual folders while preserving existing table column filters and grouping.
