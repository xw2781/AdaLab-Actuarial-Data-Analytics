...
- ArcBot reads its behavior prompt from the configured Server Connection root at `config\arcbot_prompt.md` so teams can share and adjust the prompt without rebuilding the app; when the server prompt is missing, the Electron host seeds it from the bundled default prompt before the request runs.
