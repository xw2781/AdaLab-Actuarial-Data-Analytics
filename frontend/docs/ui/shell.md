...
- `ui/index.html`: external scripts `/ui/shell/ui_shell.js?v=20260515c`; inline imports _none_.
...
- Shell pages import the shared `ui/shared/scrollbars.css` WebKit scrollbar treatment so ArcBot chat/debug/context scroll areas use the same scrollbar visual style as Dataset/DFM.
- ArcBot chat uses a pro-grey minimalist panel with flat controls, low-contrast circular user-initial avatars and ArcBot Mini assistant avatars in the conversation, an Online/Offline stamp and refresh control beside the ArcBot title, right-side icon-only New Chat and History actions, a rounded composer frame with file-type preview attachment cards, a plus menu with an Attach files and more action for adding common text-like files as one-request context, centered App Context and Edit Automatically / Read Only mode controls, a bottom App Context toggle that previews the active tab/file on hover when enabled and prevents ArcBot from receiving active app contents when disabled, a send button that switches to a cancel action while a Codex request is running, an in-chat four-stage work summary card that streams request understanding, app-content scanning, execution/modification, and finalization states while running and collapses when complete, a resizable chat panel, auto-growing frameless composer input, and a title-bar debug drawer with copyable raw stdout/stderr and host event logs.
