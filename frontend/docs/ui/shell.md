...
- `ui/index.html`: external scripts `/ui/shell/ui_shell.js?v=20260515b`; inline imports _none_.
...
- ArcBot chat uses a pro-grey minimalist panel with flat controls, low-contrast circular user-initial avatars and ArcBot Mini assistant avatars in the conversation, an Online/Offline stamp and refresh control beside the ArcBot title, right-side icon-only New Chat and History actions, a rounded composer frame with file-type preview attachment cards, a plus menu with an Attach files and more action for adding common text-like files as one-request context, centered App Context, model, and Edit Automatically / Read Only mode controls, a bottom App Context toggle that previews the active tab/file on hover when enabled and prevents ArcBot from receiving active app contents when disabled, a send button that switches to a cancel action while a Codex request is running, an in-chat elapsed-time work summary card shown above the assistant result that stays expanded while a request is running and collapses when complete, a resizable chat panel, auto-growing frameless composer input, and a title-bar debug drawer with copyable raw stdout/stderr and host event logs.
...
- Persists ArcBot chat sessions as local JSON under Electron user data, including messages, activity, mode/model, archived state, latest active-tab context summary, and estimated context usage; local ArcBot edit-session JSON copies are written under the user `Documents\ArcRho\ArcBot\sessions` folder, and latest ArcBot edit metadata is stored under Electron user data so a later `revert latest` request can restore the backed-up method JSON when the file has not changed again.
- ArcBot panel resizing clamps saved and live widths to a minimum that keeps the composer controls visible without clipping, and each app session's first ArcBot panel open starts at that minimum width while preserving the saved height.
<!-- MANUAL:END -->
