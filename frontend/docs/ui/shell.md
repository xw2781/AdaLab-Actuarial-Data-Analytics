...
- The menubar includes `Macro` after Settings. Clicking it opens a shell-owned floating macro panel with the same in-app titlebar/drag behavior used by the ArcBot panel; it lists `.py` files from the user scripting directory, shows the standardized macro description block, reports run/edit results through the app status bar, and can run the selected macro against the active DFM tab or open it in a new Scripting Console tab for code viewing/editing.
